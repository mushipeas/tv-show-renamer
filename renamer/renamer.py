...
class Renamer:
    ...
    def search_series_name(self, show_title: str, year: str = " ") -> list:
        try:
            ...
            try:
                ...
                print(
                    "Err     Show-name search returned zero results for {}.".format(
                        search_term
                    )
                )
                raise
    ...
    def get_ep_tvdb_info(self, regex_data: dict) -> "TVDB Show obj, Episode obj":
        try:
            bm_tvdb_series = self._best_match_series(
                regex_data["show_title"], regex_data["show_year"]
            )
        except:
            print(
                'Err      Show-name TVDB search returned zero results for "{}" parsed from file: "{}"'.format(
                    regex_data["show_title"], regex_data["original_file_name"]
                )
            )
        try:
            tvdb_episode = bm_tvdb_series[regex_data["season_no"]][
    ...
    def format_filename(
        ...
        return format_str.format(
            series_title=series_title, s_no=s_no, ep_no=ep_no, ep_name=ep_name, ext=ext
        )

    def get_new_filename(self, tvdb_series, tvdb_episode, extension):
        new_filename = self.format_filename(
    ...
    def make_winsafe(self, unsafe_filename: str):
        ...
        safe_filename_interm = re.sub(
            "[{}]".format(re.escape(unsafe_char_sp)), repl_sp, unsafe_filename
        )
    ...
    def get_relative_pathname(self, orig_filename: str):
        ...
        if regex_data:
            ...
            if bm_tvdb_series and tvdb_episode:
                ...
                season_dir = self.SEASON_DIR_TEMPLATE.format(
                    tvdb_episode["airedSeason"]
                )
                new_filename = self.get_new_filename(
                    bm_tvdb_series, tvdb_episode, regex_data["extension"]
                )
